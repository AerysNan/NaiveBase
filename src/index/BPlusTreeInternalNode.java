...
import static global.Global.fanout;

final class BPlusTreeInternalNode<K extends Comparable<K>, V> extends BPlusTreeNode<K, V> {

    ArrayList<BPlusTreeNode<K, V>> children;

    BPlusTreeInternalNode(int size) {
        keys = new ArrayList<>(Collections.nCopies((int) (1.5 * fanout) + 1, null));
        children = new ArrayList<>((Collections.nCopies((int) (1.5 * fanout) + 2, null)));
        this.nodeSize = size;
    }
